...
- `channels.mattermost.groups.<channelId>.requireMention`: sobrescritura por canal del requisito de mención (`"*"` como predeterminado).
...
- Se prefieren destinos `chat_id:<id>`. Usa `imsg chats --limit 20` para listar los chats.
...
- Sobrescrituras de entorno: `OPENCLAW_AGENT_RUNTIME=<id|auto|pi>` sobrescribe `runtime`; `OPENCLAW_AGENT_HARNESS_FALLBACK=none` desactiva el respaldo a PI para ese proceso.
...
- **`maintenance`**: controles de limpieza + retención del almacén de sesiones.
  ...
  - `resetArchiveRetention`: retención para archivos de transcripción `*.reset.<timestamp>`. Usa `pruneAfter` por defecto; establece `false` para desactivar.
...
- `entries.<skillKey>.apiKey`: conveniencia para Skills que declaran una variable de entorno primaria (cadena de texto sin formato u objeto SecretRef).
...
- `channels.<provider>.healthMonitor.enabled`: opt-out por canal de los reinicios del monitor de salud manteniendo habilitado el monitor global.
