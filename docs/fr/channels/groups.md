...
title: Groupes
x-i18n:
    generated_at: "2026-04-07T06:49:24Z"
    model: gpt-5.4
    provider: openai
...
> TL;DR
>
> - L’accès en **message privé** est contrôlé par `*.allowFrom`.
> - L’accès en **groupe** est contrôlé par `*.groupPolicy` + les listes d’autorisation (`*.groups`, `*.groupAllowFrom`).
...
Par défaut, OpenClaw privilégie un comportement de discussion normal et conserve le contexte en grande partie tel qu’il a été reçu. Cela signifie que les listes d’autorisation déterminent principalement qui peut déclencher des actions, et non une frontière universelle de masquage pour chaque citation ou extrait historique.

Le comportement actuel dépend du canal :

- Certains canaux appliquent déjà un filtrage basé sur l’expéditeur pour le contexte supplémentaire dans certains parcours (par exemple l’amorçage de fils Slack, les recherches de réponse/fil Matrix).
...
| -------------------------------------------- | ---------------------------------------------------------- |
| Autoriser tous les groupes mais ne répondre que sur @mention | `groups: { "*": { requireMention: true } }`                |
...
- **Groupes** : sandbox + outils de messagerie restreints (Docker)

> Si vous avez besoin d’espaces de travail/personas réellement séparés (le trafic « personnel » et « public » ne doivent jamais se mélanger), utilisez un second agent + des liaisons. Voir [Routage multi-agent](/fr/concepts/multi-agent).

Exemple (messages privés sur l’hôte, groupes sandboxés + outils de messagerie uniquement) :
...
- Déboguer pourquoi un outil est bloqué : [Sandbox vs politique d’outils vs Elevated](/fr/gateway/sandbox-vs-tool-policy-vs-elevated)
- Détails sur les montages bind : [Sandboxing](/fr/gateway/sandboxing#custom-bind-mounts)

## Libellés d’affichage

- Les libellés de l’interface utilisent `displayName` lorsqu’il est disponible, au format `<channel>:<token>`.
- `#room` est réservé aux salons/canaux ; les discussions de groupe utilisent `g-<slug>` (minuscules, espaces remplacés par `-`, conserver `#@+._-`).
...
- Les approbations d’appairage en message privé (entrées du stockage `*-allowFrom`) s’appliquent uniquement à l’accès en message privé ; l’autorisation des expéditeurs en groupe reste explicitement liée aux listes d’autorisation de groupe.
...
Les messages de groupe nécessitent une mention, sauf si cela est remplacé groupe par groupe. Les valeurs par défaut se trouvent par sous-système sous `*.groups."*"`.
...
- `toolsBySender` : remplacements par expéditeur au sein du groupe.
  ...
  Les clés héritées sans préfixe sont toujours acceptées et sont associées uniquement comme `id:`.
...
- Les restrictions d’outils par groupe/canal s’appliquent en plus de la politique d’outils globale/de l’agent (un refus reste prioritaire).
- Certains canaux utilisent un niveau d’imbrication différent pour les salons/canaux (par exemple Discord `guilds.*.channels.*`, Slack `channels.*`, Microsoft Teams `teams.*.channels.*`).
...
Confusion fréquente : l’approbation d’appairage en message privé n’est pas la même chose que l’autorisation de groupe.
Pour les canaux qui prennent en charge l’appairage en message privé, le stockage d’appairage ne déverrouille que les messages privés. Les commandes de groupe exigent toujours une autorisation explicite des expéditeurs de groupe à partir des listes d’autorisation de configuration telles que `groupAllowFrom` ou la solution de repli de configuration documentée pour ce canal.
...
2. Autoriser seulement certains groupes (WhatsApp)
...
Le propriétaire est déterminé par `channels.whatsapp.allowFrom` (ou l’E.164 du bot lui-même si non défini). Envoyez la commande sous forme de message autonome. Les autres surfaces ignorent actuellement `/activation`.
...
- BlueBubbles peut enrichir de manière facultative les participants de groupe macOS sans nom à partir de la base de données locale Contacts avant de renseigner `GroupMembers`. Cette option est désactivée par défaut et ne s’exécute qu’après le passage normal du filtrage de groupe.

Le prompt système de l’agent inclut une introduction de groupe au premier tour d’une nouvelle session de groupe. Elle rappelle au modèle de répondre comme un humain, d’éviter les tableaux Markdown, de réduire au minimum les lignes vides, de respecter l’espacement normal d’une conversation et d’éviter de taper des séquences littérales `\n`.
