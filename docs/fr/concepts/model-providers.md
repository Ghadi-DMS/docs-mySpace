...
x-i18n:
    generated_at: "2026-04-07T06:50:33Z"
    model: gpt-5.4
    provider: openai
...
- Les règles d’exécution de repli, les sondes de refroidissement et la persistance des remplacements de session sont
...
- `models.providers.*.models[].contextWindow` correspond aux métadonnées natives du modèle ;
  `models.providers.*.models[].contextTokens` correspond au plafond d’exécution effectif.
- Les plugins de fournisseur peuvent injecter des catalogues de modèles via `registerProvider({ catalog })` ;
  ...
  `models.json`.
- Les manifestes de fournisseur peuvent déclarer `providerAuthEnvVars` pour que les sondes d’authentification génériques basées sur les variables d’environnement
  n’aient pas besoin de charger l’exécution du plugin. La table restante de variables d’environnement du cœur
  ne sert plus qu’aux fournisseurs non plugin / du cœur et à quelques cas de priorité génériques
  comme l’onboarding Anthropic donnant la priorité à la clé API.
- Les plugins de fournisseur peuvent aussi posséder le comportement d’exécution du fournisseur via
  ...
  `onModelSelected`.
- Remarque : les `capabilities` d’exécution du fournisseur sont des métadonnées partagées du moteur d’exécution (famille de fournisseur, particularités de transcription/outillage, indications de transport/cache). Ce n’est pas la
...
Les plugins de fournisseur peuvent désormais posséder la plupart des logiques spécifiques au fournisseur, tandis qu’OpenClaw conserve
...
- `normalizeTransport` : le fournisseur normalise `api` / `baseUrl` de la famille de transport
  ...
  puis les autres plugins de fournisseur compatibles avec ce hook jusqu’à ce que l’un d’eux modifie réellement
  le transport
- `normalizeConfig` : le fournisseur normalise la configuration `models.providers.<id>` avant
  ...
  plugins de fournisseur compatibles avec ce hook jusqu’à ce que l’un d’eux modifie réellement la configuration. Si aucun
  hook de fournisseur ne réécrit la configuration, les assistants Google intégrés
  normalisent toujours les entrées de fournisseur Google prises en charge.
- `applyNativeStreamingUsageCompat` : le fournisseur applique des réécritures de compatibilité d’utilisation du streaming natif pilotées par le point de terminaison pour les fournisseurs de configuration
...
- `resolveSyntheticAuth` : le fournisseur peut exposer la disponibilité d’une authentification locale / auto-hébergée ou autre
  basée sur la configuration sans conserver de secrets en clair
...
- `resolveDynamicModel` : le fournisseur accepte des identifiants de modèle pas encore présents dans le
...
- `contributeResolvedModelCompat` : le fournisseur apporte des indicateurs de compatibilité pour ses
  modèles fournisseurs même lorsqu’ils arrivent via un autre transport compatible
- `capabilities` : le fournisseur publie les particularités de transcription/outillage/famille de fournisseur
- `normalizeToolSchemas` : le fournisseur nettoie les schémas d’outils avant que le moteur d’exécution intégré
  ne les voie
...
- `createEmbeddingProvider` : le fournisseur possède le comportement des embeddings mémoire lorsqu’il
  relève davantage du plugin de fournisseur que du commutateur central d’embeddings
...
- `refreshOAuth` : le fournisseur gère lui-même l’actualisation OAuth lorsque les
  mécanismes d’actualisation partagés `pi-ai` ne suffisent pas
- `buildAuthDoctorHint` : le fournisseur ajoute une indication de réparation quand l’actualisation OAuth
  échoue
...
- `buildMissingAuthMessage` : le fournisseur remplace le message d’erreur générique du magasin d’authentification
...
- `suppressBuiltInModel` : le fournisseur masque les lignes amont obsolètes et peut renvoyer une
  erreur contrôlée par le fournisseur pour les échecs de résolution directe
...
- `isModernModelRef` : le fournisseur gère lui-même la correspondance des modèles préférés en direct / de test fumée
...
- `resolveUsageAuth` : le fournisseur résout les identifiants d’utilisation/quota pour `/usage`
  et les surfaces associées d’état/rapport
- `fetchUsageSnapshot` : le fournisseur possède la récupération/analyse du point de terminaison d’utilisation tandis que
  le cœur conserve l’enveloppe récapitulative et la mise en forme
- `onModelSelected` : le fournisseur exécute des effets secondaires après la sélection comme
  la télémétrie ou la tenue de session possédée par le fournisseur

Exemples actuellement groupés :

- `anthropic` : repli de compatibilité descendante vers Claude 4.6, indications de réparation d’authentification, récupération
  du point de terminaison d’utilisation, métadonnées de TTL de cache/famille de fournisseur, et valeurs par défaut globales
  de configuration sensibles à l’authentification
- `amazon-bedrock` : détection du dépassement de contexte possédée par le fournisseur et classification des
  raisons de bascule pour les erreurs Bedrock spécifiques de limitation / non-prêt, plus
  la famille partagée `anthropic-by-model` pour les garde-fous de politique de rejeu propres à Claude
  sur le trafic Anthropic
- `anthropic-vertex` : garde-fous de politique de rejeu propres à Claude sur le trafic
  de messages Anthropic
- `openrouter` : identifiants de modèle pass-through, enveloppes de requête, indications de capacité de fournisseur,
  assainissement des signatures de réflexion Gemini sur le trafic proxy Gemini, injection de raisonnement proxy
  via la famille de flux `openrouter-thinking`, transfert des métadonnées
  de routage et politique de TTL du cache
- `github-copilot` : onboarding / connexion d’appareil, repli de modèle de compatibilité descendante,
  indications de transcription Claude-thinking, échange de jetons d’exécution et récupération
  du point de terminaison d’utilisation
- `openai` : repli de compatibilité descendante vers GPT-5.4, normalisation directe du transport OpenAI,
  indications d’authentification manquante tenant compte de Codex, suppression de Spark, lignes de catalogue synthétiques
  OpenAI/Codex, politique de réflexion/modèle en direct, normalisation des alias de jetons d’utilisation
  (`input` / `output` et familles `prompt` / `completion`), la famille de flux partagée `openai-responses-defaults`
  pour les enveloppes natives OpenAI/Codex, métadonnées de famille de fournisseur, enregistrement groupé
  du fournisseur de génération d’images pour `gpt-image-1` et enregistrement groupé
  du fournisseur de génération vidéo pour `sora-2`
- `google` et `google-gemini-cli` : repli de compatibilité descendante vers Gemini 3.1,
  validation native du rejeu Gemini, assainissement du rejeu d’amorçage, mode de sortie de raisonnement
  balisé, correspondance des modèles modernes, enregistrement groupé du fournisseur de génération d’images
  pour les modèles Gemini image-preview, et enregistrement groupé
  du fournisseur de génération vidéo pour les modèles Veo ; Gemini CLI OAuth gère aussi
  le formatage des jetons de profil d’authentification, l’analyse des jetons d’utilisation et la récupération
...
- `kilocode` : transport partagé, en-têtes de requête possédés par le plugin, normalisation de la charge utile de raisonnement,
  assainissement des signatures de réflexion Gemini en proxy et politique de TTL
  du cache
- `zai` : repli de compatibilité descendante vers GLM-5, valeurs par défaut `tool_stream`, politique de TTL
  du cache, politique de réflexion binaire/modèle en direct, et authentification d’utilisation + récupération de quota ;
...
- `xai` : normalisation native du transport Responses, réécritures d’alias `/fast` pour
  ...
  charges utiles de raisonnement spécifique à xAI, et enregistrement groupé du fournisseur de génération vidéo
  pour `grok-imagine-video`
- `mistral` : métadonnées de capacité possédées par le plugin
- `opencode` et `opencode-go` : métadonnées de capacité possédées par le plugin plus
  assainissement des signatures de réflexion Gemini en proxy
- `alibaba` : catalogue vidéo possédé par le plugin pour les références directes de modèle Wan
  telles que `alibaba/wan2.6-t2v`
- `byteplus` : catalogues possédés par le plugin plus enregistrement groupé du fournisseur de génération vidéo
  pour les modèles text-to-video / image-to-video Seedance
- `fal` : enregistrement groupé du fournisseur de génération vidéo pour un fournisseur tiers hébergé
...
- `qwen` : catalogues possédés par le plugin pour les modèles texte plus enregistrements partagés
  des fournisseurs d’analyse média et de génération vidéo pour ses surfaces
  multimodales ; la génération vidéo Qwen utilise les points de terminaison vidéo DashScope Standard
  avec les modèles Wan groupés tels que `wan2.6-t2v` et `wan2.7-r2v`
- `runway` : enregistrement de fournisseur de génération vidéo possédé par le plugin pour les modèles natifs
  basés sur des tâches Runway tels que `gen4.5`
- `minimax` : catalogues possédés par le plugin, enregistrement groupé du fournisseur de génération vidéo
  ...
  et logique d’authentification/cliché d’utilisation
- `together` : catalogues possédés par le plugin plus enregistrement groupé du fournisseur de génération vidéo
...
qui nécessite un exécuteur de requêtes totalement personnalisé relève d’une surface d’extension
distincte et plus profonde.

## Rotation des clés API

- Prend en charge la rotation générique de fournisseur pour certains fournisseurs.
...
- `openai/gpt-5.3-codex-spark` est intentionnellement supprimé dans OpenClaw parce que l’API OpenAI en direct le rejette ; Spark est traité comme réservé à Codex
...
- Remarque Anthropic : l’équipe Anthropic nous a indiqué que l’utilisation de type Claude CLI dans OpenClaw est de nouveau autorisée, donc OpenClaw considère la réutilisation de Claude CLI et l’utilisation de `claude -p` comme autorisées pour cette intégration, sauf si Anthropic publie une nouvelle politique.
...
- Remarque de politique : l’OAuth OpenAI Codex est explicitement pris en charge pour les outils/flux de travail externes comme OpenClaw.
...
- [Qwen Cloud](/fr/providers/qwen) : surface du fournisseur Qwen Cloud plus mappage des points de terminaison Alibaba DashScope et Coding Plan
...
- Attention : Gemini CLI OAuth dans OpenClaw est une intégration non officielle. Certains utilisateurs ont signalé des restrictions de compte Google après utilisation de clients tiers. Consultez les conditions de Google et utilisez un compte non critique si vous choisissez de continuer.
- Gemini CLI OAuth est livré comme partie du plugin `google` groupé.
  ...
  - Les réponses JSON Gemini CLI sont analysées depuis `response` ; l’utilisation retombe sur
...
- Le catalogue statique de repli inclut `kilocode/kilo/auto` ; la découverte en direct
  via `https://api.kilo.ai/api/gateway/models` peut étendre davantage le catalogue
  d’exécution.
- Le routage amont exact derrière `kilocode/kilo/auto` relève de Kilo Gateway,
...
- OpenClaw applique les en-têtes d’attribution d’application documentés par OpenRouter seulement lorsque
  la requête cible réellement `openrouter.ai`
- Les marqueurs `cache_control` Anthropic propres à OpenRouter sont eux aussi limités
  aux routes OpenRouter vérifiées, pas à des URL proxy arbitraires
- OpenRouter reste sur le chemin compatible OpenAI de style proxy, donc la mise en forme
  de requête réservée à OpenAI natif (`serviceTier`, `store` de Responses,
  indications de cache de prompt, charges utiles de compatibilité de raisonnement OpenAI) n’est pas transmise
- Les références OpenRouter reposant sur Gemini conservent seulement
  l’assainissement des signatures de réflexion Gemini en proxy ; la validation native du rejeu Gemini et les réécritures d’amorçage restent désactivées
- Kilo Gateway : `kilocode` (`KILOCODE_API_KEY`)
- Exemple de modèle : `kilocode/kilo/auto`
- Les références Kilo reposant sur Gemini conservent le même chemin
  d’assainissement des signatures de réflexion Gemini en proxy ; `kilocode/kilo/auto` et les autres indications ne prenant pas en charge le raisonnement en proxy
  ignorent l’injection de raisonnement en proxy
...
- L’onboarding MiniMax / la configuration par clé API écrit des définitions explicites du modèle M2.7 avec
  `input: ["text", "image"]` ; le catalogue de fournisseur groupé conserve les références de discussion
...
Utilisez `models.providers` (ou `models.json`) pour ajouter des fournisseurs **personnalisés** ou des
proxys compatibles OpenAI/Anthropic.

Beaucoup des plugins de fournisseur groupés ci-dessous publient déjà un catalogue par défaut.
Utilisez des entrées explicites `models.providers.<id>` seulement lorsque vous voulez remplacer
...
Dans les sélecteurs de modèle d’onboarding / configuration, le choix d’authentification BytePlus préfère à la fois
...
- La compréhension d’images est le `MiniMax-VL-01` possédé par le plugin sur les deux chemins d’authentification MiniMax
...
`openclaw onboard` ainsi qu’au sélecteur de modèles. Voir [/providers/ollama](/fr/providers/ollama)
pour l’onboarding, le mode cloud/local et la configuration personnalisée.

### vLLM

vLLM est livré comme plugin de fournisseur groupé pour les serveurs locaux / auto-hébergés
compatibles OpenAI :
...
Pour activer l’auto-détection locale (n’importe quelle valeur fonctionne si votre serveur n’impose pas d’authentification) :
...
SGLang est livré comme plugin de fournisseur groupé pour des serveurs auto-hébergés
rapides compatibles OpenAI :

- Fournisseur : `sglang`
- Authentification : facultative (selon votre serveur)
- URL de base par défaut : `http://127.0.0.1:30000/v1`

Pour activer l’auto-détection locale (n’importe quelle valeur fonctionne si votre serveur n’impose pas
d’authentification) :
...
- Pour `api: "openai-completions"` sur des points de terminaison non natifs (toute `baseUrl` non vide dont l’hôte n’est pas `api.openai.com`), OpenClaw force `compat.supportsDeveloperRole: false` afin d’éviter les erreurs 400 du fournisseur pour les rôles `developer` non pris en charge.
- Les routes proxy compatibles OpenAI de style proxy ignorent aussi la mise en forme de requête réservée à OpenAI natif :
