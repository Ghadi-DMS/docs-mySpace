...
en ligne de Microsoft Edge via la bibliothèque `node-edge-tts`. Il s’agit d’un service hébergé (et non
...
considérez-le comme fourni au mieux. Si vous avez besoin de limites garanties et d’un support, utilisez OpenAI
ou ElevenLabs.
...
- Si `provider` est **non défini**, OpenClaw utilise le premier fournisseur vocal configuré selon l’ordre de sélection automatique du registre.
...
- Les valeurs `apiKey` utilisent comme repli les variables d’environnement (`ELEVENLABS_API_KEY`/`XI_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`).
...
- `providers.microsoft.enabled` : autoriser l’utilisation de la voix Microsoft (par défaut `true` ; pas de clé API).
...
- **Autres canaux** : MP3 (`mp3_44100_128` depuis ElevenLabs, `mp3` depuis OpenAI).
  - 44,1 kHz / 128 kbps est l’équilibre par défaut pour la clarté de la voix.
...
Il n’existe qu’une seule commande : `/tts`.
...
- Les échecs API OpenAI et ElevenLabs incluent désormais le détail d’erreur analysé du fournisseur et l’identifiant de requête (lorsqu’il est renvoyé par le fournisseur), ce qui apparaît dans les erreurs/journaux TTS.
...
l’audio est envoyé comme message vocal plutôt que comme pièce jointe de fichier.
