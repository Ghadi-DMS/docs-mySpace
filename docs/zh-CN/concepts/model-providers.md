...
title: 模型提供商
x-i18n:
    generated_at: "2026-04-07T09:39:07Z"
    model: gpt-5.4
    provider: openai
...
- 运行时回退规则、冷却探测和会话覆盖持久化记录在 [/concepts/model-failover](/zh-CN/concepts/model-failover)。
...
- 提供商清单可以声明 `providerAuthEnvVars`，这样基于通用环境变量的
  凭证探测就不需要加载插件运行时。剩余的核心环境变量映射
  现在仅用于非插件/核心提供商，以及少数通用优先级场景，
  例如 Anthropic 的“API 密钥优先”新手引导。
...
- 注意：提供商运行时 `capabilities` 是共享运行器元数据（提供商
  家族、转录/工具特殊行为、传输/缓存提示）。它不同于
  [公共能力模型](/zh-CN/plugins/architecture#public-capability-model)，
  后者描述的是插件注册了什么（文本推理、语音等）。

## 插件自有的提供商行为

提供商插件现在可以拥有大部分特定于提供商的逻辑，而 OpenClaw 保留
通用推理循环。
...
- `normalizeTransport`：提供商在通用模型组装前标准化传输族 `api` / `baseUrl`；
  OpenClaw 会先检查匹配的提供商，
  然后检查其他支持 hook 的提供商插件，直到某个插件实际更改了
  传输
- `normalizeConfig`：提供商在运行时使用前标准化 `models.providers.<id>` 配置；
  OpenClaw 会先检查匹配的提供商，然后检查其他
  支持 hook 的提供商插件，直到某个插件实际更改了配置。如果没有
  提供商 hook 重写配置，内置的 Google 系辅助逻辑仍会
...
- `contributeResolvedModelCompat`：即使提供商的厂商模型通过另一种兼容传输到达，
  该提供商也可以为其贡献兼容性标记
- `capabilities`：提供商发布转录/工具/提供商家族的特殊行为
- `normalizeToolSchemas`：提供商在嵌入式
  运行器看到工具模式之前对其进行清理
- `inspectToolSchemas`：提供商在标准化后公开特定于传输的模式警告
...
- `resolveTransportTurnState`：提供商提供每轮的原生传输
  请求头或元数据
- `resolveWebSocketSessionPolicy`：提供商提供原生 WebSocket 会话
  请求头或会话冷却策略
- `createEmbeddingProvider`：当记忆嵌入行为
  更适合放在提供商插件中而不是核心嵌入切换板时，由提供商拥有它
- `formatApiKey`：提供商将已存储的凭证配置文件格式化为传输期望的运行时
  `apiKey` 字符串
...
- `matchesContextOverflowError`：提供商识别提供商特有的
  上下文窗口溢出错误，而通用启发式方法可能会漏掉这些错误
- `classifyFailoverReason`：提供商将提供商特有的原始传输/API
  错误映射为回退原因，例如速率限制或过载
- `isCacheTtlEligible`：提供商决定哪些上游模型 id 支持提示缓存 TTL
- `buildMissingAuthMessage`：提供商用特定于提供商的恢复提示
  替换通用的凭证存储错误
- `suppressBuiltInModel`：提供商隐藏过时的上游条目，并且可以在直接解析失败时
  返回厂商自有错误
- `augmentModelCatalog`：提供商在发现和配置合并后
  追加合成/最终目录条目
- `isBinaryThinking`：提供商拥有二元开/关思考 UX
- `supportsXHighThinking`：提供商选择将特定模型纳入 `xhigh`
- `resolveDefaultThinkingLevel`：提供商拥有某个
  模型家族的默认 `/think` 策略
- `applyConfigDefaults`：提供商根据凭证模式、环境变量或模型家族，
  在配置具体化期间应用特定于提供商的全局默认值
- `isModernModelRef`：提供商拥有实时/冒烟测试首选模型匹配
- `prepareRuntimeAuth`：提供商将已配置的凭证转换为短时效
  运行时令牌
...
- `anthropic`：Claude 4.6 向前兼容回退、凭证修复提示、用量
  端点获取、cache-TTL/提供商家族元数据，以及具备凭证感知能力的全局
  配置默认值
- `amazon-bedrock`：提供商自有的上下文溢出匹配，以及针对 Bedrock 特有节流/未就绪错误的
  回退原因分类，另外还包括
  共享的 `anthropic-by-model` 重放家族，用于 Anthropic 流量上仅限 Claude 的重放策略
  防护
- `anthropic-vertex`：针对 Anthropic-message
  流量的仅限 Claude 的重放策略防护
- `openrouter`：透传模型 id、请求包装器、提供商能力
  提示、代理 Gemini 流量上的 Gemini thought-signature 清理、通过 `openrouter-thinking` 流家族进行的代理
  推理注入、路由
...
- `openai`：GPT-5.4 向前兼容回退、直接 OpenAI 传输
  标准化、具备 Codex 感知能力的缺失凭证提示、Spark 抑制、合成
  ...
  包装器、提供商家族元数据、`gpt-image-1` 的内置图像生成提供商
  注册，以及 `sora-2` 的内置视频生成提供商
  注册
- `google` 和 `google-gemini-cli`：Gemini 3.1 向前兼容回退、
  原生 Gemini 重放校验、引导重放清理、带标签的
  推理输出模式、现代模型匹配、Gemini 图像预览模型的内置图像生成
  提供商注册，以及 Veo 模型的内置
  视频生成提供商注册；Gemini CLI OAuth 还
  拥有 auth-profile 令牌格式化、用量令牌解析，以及面向用量界面的配额端点
  获取
- `moonshot`：共享传输、插件自有的 thinking 负载标准化
- `kilocode`：共享传输、插件自有请求头、推理负载
  标准化、代理 Gemini thought-signature 清理，以及 cache-TTL
  策略
- `zai`：GLM-5 向前兼容回退、`tool_stream` 默认值、cache-TTL
  策略、二元 thinking/live-model 策略，以及用量凭证 + 配额获取；
  未知 `glm-5*` id 会从内置的 `glm-4.7` 模板合成
- `xai`：原生 Responses 传输标准化、针对
  Grok 快速变体的 `/fast` 别名重写、默认 `tool_stream`、xAI 特有的工具模式 /
  推理负载清理，以及 `grok-imagine-video` 的内置视频生成提供商
  注册
- `mistral`：插件自有能力元数据
- `opencode` 和 `opencode-go`：插件自有能力元数据，以及
  代理 Gemini thought-signature 清理
- `alibaba`：插件自有视频生成目录，适用于直接 Wan 模型引用，
  例如 `alibaba/wan2.6-t2v`
- `byteplus`：插件自有目录，以及面向 Seedance 文生视频/图生视频模型的内置视频生成提供商
  注册
- `fal`：内置视频生成提供商注册，适用于托管的第三方
  图像生成提供商注册（用于 FLUX 图像模型），以及面向托管第三方视频模型的内置
  视频生成提供商注册
- `cloudflare-ai-gateway`、`huggingface`、`kimi`、`nvidia`、`qianfan`、
  `stepfun`、`synthetic`、`venice`、`vercel-ai-gateway` 和 `volcengine`：
  仅插件自有目录
- `qwen`：文本模型的插件自有目录，以及其多模态界面的共享
  media-understanding 和视频生成提供商注册；
  Qwen 视频生成使用 Standard DashScope 视频
  端点，以及内置的 Wan 模型，例如 `wan2.6-t2v` 和 `wan2.7-r2v`
- `runway`：针对原生
  Runway 任务型模型（如 `gen4.5`）的插件自有视频生成提供商注册
- `minimax`：插件自有目录、Hailuo 视频模型的内置视频生成提供商
  注册、`image-01` 的内置图像生成提供商
  注册、混合 Anthropic/OpenAI 重放策略
  选择，以及用量凭证/快照逻辑
- `together`：插件自有目录，以及 Wan 视频模型的内置视频生成提供商
  注册
...
这涵盖了仍然适配 OpenClaw 常规传输方式的提供商。需要完全自定义请求执行器的提供商
则属于另一个更深层的扩展接口。
...
- 密钥选择顺序会保留优先级并去重。
- 只有在速率限制响应时，请求才会使用下一个密钥重试（例如
  `429`、`rate_limit`、`quota`、`resource exhausted`、`Too many
concurrent requests`、`ThrottlingException`、`concurrency limit reached`、
  `workers_ai ... quota limit exceeded`，或周期性的使用量限制消息）。
- 非速率限制失败会立即失败；不会尝试进行密钥轮换。
...
- 当你想使用明确的层级而不是共享的 `/fast` 开关时，请使用 `params.serviceTier`
- 隐藏的 OpenClaw 归属请求头（`originator`、`version`、
  `User-Agent`）仅应用于发往 `api.openai.com` 的原生 OpenAI 流量，不适用于
  通用 OpenAI 兼容代理
...
- 直接公共 Anthropic 请求支持共享的 `/fast` 开关和 `params.fastMode`，包括发送到 `api.anthropic.com` 的 API 密钥和 OAuth 身份验证流量；OpenClaw 会将其映射到 Anthropic `service_tier`（`auto` 与 `standard_only`）
- Anthropic 说明：Anthropic 员工告诉我们，OpenClaw 风格的 Claude CLI 使用方式再次被允许，因此除非 Anthropic 发布新政策，否则 OpenClaw 会将 Claude CLI 复用和 `claude -p` 用法视为该集成的许可方式。
...
- 当 Codex OAuth 目录公开时，`openai-codex/gpt-5.3-codex-spark` 仍然可用；是否可用取决于权限
...
- 注意：OpenClaw 中的 Gemini CLI OAuth 是非官方集成。一些用户报告称，在使用第三方客户端后其 Google 账号受到了限制。若你选择继续，请先查看 Google 条款并使用非关键账号。
...
- 仅当请求实际指向
  `openrouter.ai` 时，OpenClaw 才会应用 OpenRouter 文档规定的应用归属请求头
- OpenRouter 特有的 Anthropic `cache_control` 标记同样仅作用于
  已验证的 OpenRouter 路由，而不是任意代理 URL
- OpenRouter 仍保持在代理式 OpenAI 兼容路径上，因此原生
  仅 OpenAI 请求整形（`serviceTier`、Responses `store`、
  提示缓存提示、OpenAI 推理兼容负载）不会被转发
- 基于 Gemini 的 OpenRouter 引用仅保留代理 Gemini thought-signature 清理；
  原生 Gemini 重放校验和引导重写仍保持关闭
...
- MiniMax 新手引导/API 密钥设置会写入明确的 M2.7 模型定义，并带有
...
在新手引导/配置模型选择器中，Volcengine 凭证选择会优先显示 `volcengine/*` 和 `volcengine-plan/*` 条目。
如果这些模型尚未加载，
...
在新手引导/配置模型选择器中，BytePlus（国际版）凭证选择会优先显示
...
- 图像理解由插件自有的 `MiniMax-VL-01` 在两种 MiniMax 凭证路径上提供
...
当你通过 `OLLAMA_API_KEY` 选择加入时，系统会在本地 `http://127.0.0.1:11434` 检测 Ollama，
并且内置提供商插件会将 Ollama 直接添加到
`openclaw onboard` 和模型选择器中。请参见 [/providers/ollama](/zh-CN/providers/ollama)
...
要在本地选择加入自动发现（如果你的服务器不强制要求凭证，任意值都可以）：
...
- 代理式 OpenAI 兼容路由也会跳过仅原生 OpenAI 的请求
...
另请参见：[/gateway/configuration](/zh-CN/gateway/configuration)，其中包含完整配置示例。
