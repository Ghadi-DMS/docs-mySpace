...
title: 测试
x-i18n:
    generated_at: "2026-04-07T09:40:02Z"
    model: gpt-5.4
    provider: openai
...
- 完整门禁（预期应在 push 前运行）：`pnpm build && pnpm check && pnpm test`
...
- 实时套件（模型 + Gateway 网关 工具 / 图像探测）：`pnpm test:live`
- 安静地只跑一个实时文件：`pnpm test:live -- src/agents/models.profiles.live.test.ts`

提示：当你只需要一个失败用例时，优先使用下文说明的 allowlist 环境变量来缩小实时测试范围。
...
- 命令：`pnpm test`
- 配置：在现有按范围划分的 Vitest projects 之上执行十个顺序分片运行（`vitest.full-*.config.ts`）
...
- 预期：
  ...
  - 应该快速且稳定
- Projects 说明：
  - 无目标的 `pnpm test` 现在会运行十一组更小的分片配置（`core-unit-src`、`core-unit-security`、`core-unit-ui`、`core-unit-support`、`core-support-boundary`、`core-contracts`、`core-bundled`、`core-runtime`、`agentic`、`auto-reply`、`extensions`），而不是一个巨大的原生 root-project 进程。这样可以降低高负载机器上的峰值 RSS，并避免 auto-reply / 扩展工作拖慢无关套件。
  - `pnpm test --watch` 仍然使用原生根级 `vitest.config.ts` project graph，因为多分片监听循环并不现实。
  - `pnpm test`、`pnpm test:watch` 和 `pnpm test:perf:imports` 现在会优先把显式文件 / 目录目标路由到按范围划分的通道，因此 `pnpm test extensions/discord/src/monitor/message-handler.preflight.test.ts` 不需要承担完整 root project 启动的开销。
  - `pnpm test:changed` 会在 diff 仅触及可路由的源码 / 测试文件时，把变更的 git 路径扩展为相同的按范围通道；而配置 / setup 编辑仍会回退到更广泛的 root-project 重新运行。
  - 部分 `plugin-sdk` 和 `commands` 测试也会通过专门的轻量通道运行，并跳过 `test/setup-openclaw-runtime.ts`；有状态 / 运行时较重的文件则继续留在现有通道中。
  - 部分 `plugin-sdk` 和 `commands` 辅助源码文件也会在 changed 模式运行时映射到这些轻量通道中的显式同级测试，因此辅助代码修改不必为该目录重新运行整套较重测试。
  - `auto-reply` 现在有三个专用分桶：顶层核心辅助、顶层 `reply.*` 集成测试，以及 `src/auto-reply/reply/**` 子树。这让最重的 reply harness 工作不会挤占便宜的 status / chunk / token 测试。
- Embedded runner 说明：
  ...
  - 这些套件会验证作用域 id 和压缩行为仍然会通过真实的
    `run.ts` / `compact.ts` 路径流动；仅有 helper 测试不足以替代这些集成路径。
- Pool 说明：
...
- 快速本地迭代说明：
  - `pnpm test:changed` 会在变更路径可明确映射到更小套件时，路由到按范围通道。
  - `pnpm test:max` 和 `pnpm test:changed:max` 保持相同的路由行为，只是拥有更高的 worker 上限。
  - 本地 worker 自动伸缩现在有意更保守，并且在主机负载均值已经较高时也会回退，因此默认情况下多个并发 Vitest 运行对机器的影响更小。
  - 基础 Vitest 配置把 projects / config 文件标记为 `forceRerunTriggers`，这样在测试接线变更时，changed 模式重新运行仍能保持正确。
...
- 性能调试说明：
  ...
  - `pnpm test:perf:imports:changed` 会把同样的分析视图限定到自 `origin/main` 以来变更的文件。
- `pnpm test:perf:changed:bench -- --ref <git-ref>` 会把已提交 diff 的路由 `test:changed` 与原生 root-project 路径进行比较，并输出总耗时以及 macOS 最大 RSS。
- `pnpm test:perf:changed:bench -- --worktree` 会通过 `scripts/test-projects.mjs` 和根级 Vitest 配置，把当前脏工作树的变更文件列表进行路由并做基准测试。
  - `pnpm test:perf:profile:main` 会为 Vitest / Vite 启动和 transform 开销写出主线程 CPU profile。
  - `pnpm test:perf:profile:runner` 会在禁用文件并行时，为单元套件写出运行器 CPU + heap profile。
...
- 范围：
  ...
  - 通过真实的 `sandbox ssh-config` + SSH exec 走通 OpenClaw 的 OpenShell 后端
...
- 范围：
  ...
  - 捕获提供商格式变化、工具调用怪异行为、认证问题和限流行为
- 预期：
  - 设计上并不适合 CI 稳定运行（真实网络、真实提供商策略、配额、故障）
  - 会花钱 / 消耗限流额度
  - 优先运行缩小范围的子集，而不是“全部”
- 实时运行会 source `~/.profile` 以拾取缺失的 API key。
...
- `pnpm test:live` 现在默认使用更安静的模式：会保留 `[live] ...` 进度输出，但抑制额外的 `~/.profile` 提示，并静默 Gateway 网关 启动日志 / Bonjour 噪声。如果你想恢复完整启动日志，请设置 `OPENCLAW_LIVE_TEST_QUIET=0`。
- API key 轮换（按提供商）：设置 `*_API_KEYS`，使用逗号 / 分号格式，或设置 `*_API_KEY_1`、`*_API_KEY_2`（例如 `OPENAI_API_KEYS`、`ANTHROPIC_API_KEYS`、`GEMINI_API_KEYS`），也可通过 `OPENCLAW_LIVE_*_KEY` 为单次实时运行覆盖；测试会在收到限流响应时自动重试。
- 进度 / 心跳输出：
  - 实时套件现在会把进度行输出到 stderr，因此即使 Vitest 控制台捕获处于安静模式，长时间的提供商调用也能看到仍在活动。
  - `vitest.live.config.ts` 会禁用 Vitest 控制台拦截，因此提供商 / Gateway 网关 的进度行会在实时运行期间立即流出。
...
- 目标：调用连接中的 Android 节点当前**宣告的每一条命令**，并断言命令契约行为。
- 范围：
  - 依赖预置 / 手动 setup（该套件不会安装 / 运行 / 配对 app）。
  - 针对所选 Android 节点逐命令进行 Gateway 网关 `node.invoke` 校验。
- 必需的预设准备：
  - Android app 已连接并与 Gateway 网关 配对。
  - app 保持在前台。
...
- “Direct model” 告诉我们：在给定 key 下，该提供商 / 模型是否至少能正常回答。
- “Gateway smoke” 告诉我们：完整的 Gateway 网关 + 智能体 流水线是否能为该模型正常工作（会话、历史、工具、沙箱策略等）。
...
- 设置 `OPENCLAW_LIVE_MODELS=modern`（或 `all`，即 modern 的别名）才会真正运行该套件；否则会跳过，以便让 `pnpm test:live` 聚焦于 Gateway 网关 冒烟
- 如何选择模型：
...
- 为什么需要它：
  - 将“提供商 API 挂了 / key 无效”和“Gateway 网关 智能体流水线挂了”分离开来
  - 容纳小而隔离的回归测试（例如：OpenAI Responses / Codex Responses 推理重放 + 工具调用流程）

### 第 2 层：Gateway 网关 + dev 智能体 冒烟（也就是 “@openclaw” 实际做的事）
...
提示：如果你想查看你的机器上可以测试什么（以及准确的 `provider/model` id），运行：
...
- 后端特定的冒烟默认值与所属扩展的 `cli-backend.ts` 定义一起维护。
- 启用：
  - `pnpm test:live`（或者如果你直接调用 Vitest，则设置 `OPENCLAW_LIVE_TEST=1`）
  - `OPENCLAW_LIVE_CLI_BACKEND=1`
- 默认值：
  - 默认提供商 / 模型：`claude-cli/claude-sonnet-4-6`
  - 命令 / args / 图像行为来自所属 CLI 后端插件元数据。
- 覆盖项（可选）：
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `OPENCLAW_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `OPENCLAW_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` 发送真实图像附件（路径会注入到 prompt 中）。
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` 通过 CLI args 传入图像文件路径，而不是注入到 prompt。
...
- 它会从所属扩展解析 CLI 冒烟元数据，然后把匹配的 Linux CLI 包（`@anthropic-ai/claude-code`、`@openai/codex` 或 `@google/gemini-cli`）安装到可缓存的可写前缀 `OPENCLAW_DOCKER_CLI_TOOLS_DIR` 中（默认：`~/.cache/openclaw/docker-cli-tools`）。
- 实时 CLI 后端冒烟现在会对 Claude、Codex 和 Gemini 执行相同的端到端流程：文本轮次、图像分类轮次，然后通过 Gateway 网关 CLI 验证 MCP `cron` 工具调用。
...
- 目标：使用实时 ACP 智能体验证真实的 ACP 会话绑定流程：
  ...
  - 原地绑定一个合成的消息渠道会话
  - 在同一个会话上发送普通后续消息
  - 验证该后续消息落在绑定后的 ACP 会话 transcript 中
- 启用：
...
- 说明：
  - 该通道使用 Gateway 网关 的 `chat.send` 表面，并带有仅管理员可用的合成 originating-route 字段，这样测试就能附加消息渠道上下文，而无需假装对外投递。
  - 当 `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND` 未设置时，测试会为所选 ACP harness 智能体使用嵌入式 `acpx` 插件的内置智能体注册表。
...
- 默认情况下，它会按顺序对所有受支持的实时 CLI 智能体运行 ACP 绑定冒烟：`claude`、`codex`，然后 `gemini`。
- 使用 `OPENCLAW_LIVE_ACP_BIND_AGENTS=claude`、`OPENCLAW_LIVE_ACP_BIND_AGENTS=codex` 或 `OPENCLAW_LIVE_ACP_BIND_AGENTS=gemini` 可缩小矩阵。
- 它会 source `~/.profile`，将匹配的 CLI 认证材料复制进容器，把 `acpx` 安装到可写 npm 前缀中，并在缺失时安装所请求的实时 CLI（`@anthropic-ai/claude-code`、`@openai/codex` 或 `@google/gemini-cli`）。
- 在 Docker 内，运行器会设置 `OPENCLAW_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx`，这样 acpx 就会让来自已 source profile 的提供商环境变量对其子 harness CLI 保持可用。

### 推荐的实时配方

范围狭窄且明确的 allowlist 最快，也最不容易抖动：
...
- `google-gemini-cli/...` 使用你机器上的本地 Gemini CLI（独立认证 + 独立工具行为怪异）。
- Gemini API 与 Gemini CLI：
  - API：OpenClaw 通过 HTTP 调用 Google 托管的 Gemini API（API key / profile 认证）；大多数用户所说的“Gemini”就是这个。
  - CLI：OpenClaw 调用本地 `gemini` 二进制；它有自己的认证，并且行为可能不同（流式 / 工具支持 / 版本偏差）。

## 实时：模型矩阵（我们覆盖什么）

这里没有固定的“CI 模型列表”（实时测试是按需启用的），但对于一台具备 key 的开发机器，下面这些是建议定期覆盖的模型。

### 现代冒烟集合（工具调用 + 图像）

这是我们预期需要持续可用的“常见模型”运行：
...
- Mistral：`mistral/`…（从你已启用的模型里挑一个支持 “tools” 的）
...
提示：不要试图在文档里硬编码“所有模型”。权威列表始终是你机器上的 `discoverModels(...)` 返回结果 + 可用 key 的组合。

## 凭证（永远不要提交）

实时测试发现凭证的方式与 CLI 完全相同。实际含义如下：

- 如果 CLI 能用，实时测试应当能找到同样的 key。
- 如果实时测试说“没有凭证”，那就用与你调试 `openclaw models list` / 模型选择相同的方式来调试。
...
- 本地实时运行默认会把当前活动配置、每个智能体的 `auth-profiles.json` 文件、旧版 `credentials/`，以及受支持的外部 CLI 认证目录复制到一个临时测试 home 中；在该 staged 配置里，`agents.*.workspace` / `agentDir` 路径覆盖会被剥离，这样探测就不会落到你真实的主机工作区。

如果你想依赖环境变量 key（例如导出在你的 `~/.profile` 中），请在本地测试前运行 `source ~/.profile`，或者使用下面的 Docker 运行器（它们可以把 `~/.profile` 挂载到容器内）。
...
- 范围：
  ...
  - 提供商特定的 Vydra 覆盖：
    ...
    - 该文件默认运行 `veo3` 文本转视频，以及一个使用远程图像 URL 夹具的 `kling` 通道
  - 当前 `videoToVideo` 实时覆盖：
    - 仅 `runway`，并且只在所选模型是 `runway/gen4_aleph` 时
  - 当前在共享扫描中已声明但被跳过的 `videoToVideo` 提供商：
    ...
    - `openai`，因为当前共享通道无法保证组织特定的视频修补 / remix 访问权限
- 可选缩小范围：
...
- 实时模型运行器：`test:docker:live-models` 和 `test:docker:live-gateway` 只会在仓库 Docker 镜像内运行与其匹配的 profile-key 实时文件（`src/agents/models.profiles.live.test.ts` 和 `src/gateway/gateway-models.profiles.live.test.ts`），同时挂载你的本地配置目录和工作区（若挂载了 `~/.profile` 也会 source）。对应的本地入口点是 `test:live:models-profiles` 和 `test:live:gateway-profiles`。
- Docker 实时运行器默认使用更小的冒烟上限，以便完整 Docker 扫描仍然可行：
  ...
  `OPENCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`。当你明确想要更大的完整扫描时，可覆盖这些环境变量。
...
实时模型 Docker 运行器还会只 bind-mount 所需的 CLI 认证 home（如果运行未缩小范围，则挂载所有受支持的认证 home），然后在运行前把它们复制到容器 home 中，这样外部 CLI OAuth 就能刷新 token，而不会修改主机认证存储：
...
并把它 stage 到容器内的临时 workdir 中。这样既能保持运行时
镜像精简，又能让 Vitest 针对你本地的精确源码 / 配置运行。
staging 步骤会跳过大型仅本地缓存和 app 构建输出，例如
...
真实的 Telegram / Discord / 等 渠道 worker。
`test:docker:live-models` 仍然会运行 `pnpm test:live`，因此当你需要缩小范围
或在这个 Docker 通道中排除 Gateway 网关 实时覆盖时，也请一并传入
...
Open WebUI 镜像，并且 Open WebUI 可能需要完成自己的冷启动 setup。
该通道要求有一个可用的实时模型 key，而 `OPENCLAW_PROFILE_FILE`
...
权限通知，这些都通过真实的 stdio MCP bridge 进行。通知检查
会直接检查原始 stdio MCP frame，因此该冒烟验证的是 bridge
实际发出了什么，而不只是某个特定 client SDK 恰好暴露了什么。

手动 ACP 自然语言 thread 冒烟（非 CI）：

- `bun scripts/dev/discord-acp-plain-language-smoke.ts --channel <discord-channel-id> ...`
- 保留此脚本用于回归 / 调试工作流。它以后可能仍会用于 ACP thread 路由校验，因此不要删除它。
...
- `OPENCLAW_LIVE_GATEWAY_PROVIDERS=...` / `OPENCLAW_LIVE_PROVIDERS=...` 用于过滤容器中的提供商
- `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` 确保凭证来自 profile store（而不是环境变量）
- `OPENCLAW_OPENWEBUI_MODEL=...` 选择为 Open WebUI 冒烟暴露的模型
...
如果还需要完整的 Mintlify anchor 校验（包含页内标题检查），运行：`pnpm docs:check-links:anchors`。
...
- Gateway 网关 向导（WS `wizard.start` / `wizard.next`，会写入配置 + 强制认证）：`src/gateway/gateway.test.ts`（用例：“runs wizard over ws and writes auth token config”）
...
- 端到端的向导流程，用于验证会话接线和配置效果（`src/gateway/gateway.test.ts`）。

对于 Skills，当前仍缺少的内容（见 [Skills](/zh-CN/tools/skills)）：

- **决策能力：** 当 prompt 中列出 Skills 时，智能体是否会选择正确的 Skill（或避开无关的 Skill）？
- **合规性：** 智能体是否会在使用前读取 `SKILL.md`，并遵循所需步骤 / args？
- **工作流契约：** 多轮场景中，是否能断言工具顺序、会话历史延续以及沙箱边界。
...
- 如果可能，添加一个 CI 安全的回归测试（mock / stub 提供商，或者捕获准确的请求形状转换）
- 如果它天然只能在实时环境中复现（限流、认证策略），那就让实时测试保持范围狭窄，并通过环境变量按需启用
- 优先瞄准能捕捉该缺陷的最小层级：
...
- SecretRef 遍历保护规则：
  ...
  - 如果你在 `src/secrets/target-registry-data.ts` 中新增了一个 `includeInPlan` SecretRef 目标家族，请更新该测试中的 `classifyTargetClass`。该测试会在遇到未分类目标 id 时有意失败，以确保新类不会被静默跳过。
