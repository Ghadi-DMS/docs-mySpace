...
title: 插件 SDK 概览
x-i18n:
    generated_at: "2026-04-08T06:30:09Z"
    model: gpt-5.4
    provider: openai
...
每个子路径都是一个小型、独立的模块。这样可以保持快速启动，并防止循环依赖问题。对于渠道专用的入口 / 构建辅助工具，优先使用 `openclaw/plugin-sdk/channel-core`；将 `openclaw/plugin-sdk/core` 保留给更广义的总入口表面和共享辅助工具，例如 `buildChannelConfigSchema`。

不要添加或依赖以提供商命名的便捷接缝，例如 `openclaw/plugin-sdk/slack`、`openclaw/plugin-sdk/discord`、`openclaw/plugin-sdk/signal`、`openclaw/plugin-sdk/whatsapp`，或带有渠道品牌的辅助接缝。内置插件应在它们自己的 `api.ts` 或 `runtime-api.ts` barrel 中组合通用 SDK 子路径，而核心则应使用这些插件本地 barrel，或者在需求确实跨渠道时添加一个精简的通用 SDK 契约。
...
保留的内置插件辅助子路径仍会出现在该生成列表中。除非某个文档页面明确将其标为公开接口，否则应将它们视为实现细节 / 兼容性表面。
...
<AccordionGroup>
  <Accordion title="渠道子路径">
    ...
    | `plugin-sdk/channel-secret-runtime` | 精简的 secret 契约辅助工具，例如 `collectSimpleChannelFieldAssignments`、`getChannelSurface`、`pushAssignment` 和 secret 目标类型 |
  </Accordion>
  ...
  <Accordion title="提供商子路径">
    ...
    | `plugin-sdk/provider-auth-api-key` | API key 新手引导 / 配置写入辅助工具，例如 `upsertApiKeyProfile` |
    ...
    | `plugin-sdk/provider-web-search-contract` | 精简的 web-search 配置 / 凭证契约辅助工具，例如 `enablePluginInConfig`、`resolveProviderWebSearchPluginConfig` 和作用域化凭证 setter / getter |
  ...
  <Accordion title="认证与安全子路径">
    ...
    | `plugin-sdk/approval-client-runtime` | 原生 exec 审批配置 / 过滤辅助工具 |
    ...
    | `plugin-sdk/approval-handler-runtime` | 更广义的审批处理器运行时辅助工具；若更精简的 adapter / gateway 接缝已足够，应优先使用它们 |
    ...
    | `plugin-sdk/channel-secret-runtime` | 用于渠道 / 插件 secret 表面的精简 secret 契约收集辅助工具 |
  ...
  <Accordion title="运行时与存储子路径">
    ...
    | `plugin-sdk/hook-runtime` | 共享 webhook / internal hook pipeline 辅助工具 |
    ...
    | `plugin-sdk/approval-runtime` | Exec / 插件审批辅助工具、审批能力构建器、认证 / 配置辅助工具、原生路由 / 运行时辅助工具 |
    ...
    | `plugin-sdk/diagnostic-runtime` | 诊断标记和事件辅助工具 |
    | `plugin-sdk/error-runtime` | 错误图、格式化、共享错误分类辅助工具，`isApprovalNotFoundError` |
    | `plugin-sdk/fetch-runtime` | 封装后的 fetch、代理和固定查找辅助工具 |
  ...
  <Accordion title="能力与测试子路径">
    ...
    | `plugin-sdk/media-generation-runtime` | 共享媒体生成功能切换辅助工具、候选项选择和缺失模型提示 |
  ...
  <Accordion title="内存子路径">
    ...
    | `plugin-sdk/memory-core-host-engine-foundation` | 内存 host foundation engine 导出 |
    | `plugin-sdk/memory-core-host-engine-embeddings` | 内存 host embedding engine 导出 |
    | `plugin-sdk/memory-core-host-engine-qmd` | 内存 host QMD engine 导出 |
    | `plugin-sdk/memory-core-host-engine-storage` | 内存 host storage engine 导出 |
    | `plugin-sdk/memory-core-host-multimodal` | 内存 host 多模态辅助工具 |
    | `plugin-sdk/memory-core-host-query` | 内存 host 查询辅助工具 |
    | `plugin-sdk/memory-core-host-secret` | 内存 host secret 辅助工具 |
    | `plugin-sdk/memory-core-host-events` | 内存 host 事件日志辅助工具 |
    | `plugin-sdk/memory-core-host-status` | 内存 host 状态辅助工具 |
    | `plugin-sdk/memory-core-host-runtime-cli` | 内存 host CLI 运行时辅助工具 |
    | `plugin-sdk/memory-core-host-runtime-core` | 内存 host 核心运行时辅助工具 |
    | `plugin-sdk/memory-core-host-runtime-files` | 内存 host 文件 / 运行时辅助工具 |
    ...
    | `plugin-sdk/memory-host-search` | 用于搜索管理器访问的活动内存运行时门面 |
...
| `api.registerMemoryPromptSupplement(builder)`  | 增补型内存相关提示区段 |
| `api.registerMemoryCorpusSupplement(adapter)`  | 增补型内存搜索 / 读取语料 |
...
| 方法 | 注册内容 |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `api.registerContextEngine(id, factory)`   | 上下文引擎（同一时间只能有一个处于激活状态）。`assemble()` 回调会接收 `availableTools` 和 `citationsMode`，从而使引擎可以定制提示补充内容。 |
...
- `registerMemoryCapability` 还可以暴露 `publicArtifacts.listArtifacts(...)`，以便配套插件通过 `openclaw/plugin-sdk/memory-host-core` 使用导出的内存制品，而不是深入访问某个特定内存插件的私有布局。
- `registerMemoryPromptSection`、`registerMemoryFlushPlan` 和 `registerMemoryRuntime` 是兼容旧版的独占式内存插件 API。
- `registerMemoryEmbeddingProvider` 允许当前激活的内存插件注册一个或多个嵌入适配器 ID（例如 `openai`、`gemini` 或插件自定义 ID）。
...
- `before_tool_call`：返回 `{ block: true }` 是终止性的。一旦任一处理器设置该值，较低优先级的处理器将被跳过。
- `before_tool_call`：返回 `{ block: false }` 会被视为未作决定（与省略 `block` 相同），而不是覆盖。
- `before_install`：返回 `{ block: true }` 是终止性的。一旦任一处理器设置该值，较低优先级的处理器将被跳过。
- `before_install`：返回 `{ block: false }` 会被视为未作决定（与省略 `block` 相同），而不是覆盖。
- `reply_dispatch`：返回 `{ handled: true, ... }` 是终止性的。一旦任一处理器认领分发，较低优先级的处理器以及默认模型分发路径都会被跳过。
- `message_sending`：返回 `{ cancel: true }` 是终止性的。一旦任一处理器设置该值，较低优先级的处理器将被跳过。
...
通过门面加载的内置插件公共表面（`api.ts`、`runtime-api.ts`、`index.ts`、`setup-entry.ts` 以及类似公共入口文件）现在在 OpenClaw 已经运行时会优先使用当前激活的运行时配置快照。如果运行时快照尚不存在，则回退到磁盘上已解析的配置文件。

当某个辅助工具本身明确是提供商专用、且暂时不适合放入通用 SDK 子路径时，提供商插件也可以暴露一个精简的插件本地契约 barrel。当前内置示例：Anthropic 提供商将其 Claude 流辅助工具保留在自己的公共 `api.ts` / `contract-api.ts` 接缝中，而不是将 Anthropic beta-header 和 `service_tier` 逻辑提升为通用 `plugin-sdk/*` 契约。
