...
<Steps>
  ...
  <Step title="插件将一切打包在一起">
    ...
    OpenClaw 一起提供），另一些则是**外部插件**（由社区发布到 npm）。
...
| `session_status` | 轻量级 `/status` 风格回读和按会话模型覆盖 | [Session Tools](/zh-CN/concepts/session-tool) |
...
与 `/status` 一样，它可以从最新转录用量条目中回填稀疏的 token / cache 计数以及当前活动的运行时模型标签。
